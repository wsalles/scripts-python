...
class FileCopy:
    ...
    def progress_percentage(self, perc, width=None):
        ...
        # Retorna a barra de progresso como String
        return ''.join(progress_bar)

    def copy_progress(self, copied, total):
        self.copy_status = 100*copied/total
        print('\r' + self.progress_percentage(self.copy_status, width=30), end='')
        ...
        if not follow_symlinks and os.path.islink(src):
            ...
            with open(src, 'rb') as fsrc:
                with open(dst, 'wb') as fdst:
                    self.copyfileobj(fsrc, fdst, callback=self.copy_progress, total=size)
        return dst
